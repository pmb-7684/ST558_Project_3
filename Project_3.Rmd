...
```
The distribution of several quantitative variables - shares, (response variable), rate_positive_words, and rate_negative_word.

The summary provides us with information about the distribution (shape) of the variables.
...
```{r scatter share}
g <- ggplot(selectTrain, aes(x=rate_positive_words, y = shares ))+geom_point() 
...
Due to the computational memory need to create a random forest, we will be using random forest created from the ranger library.  Otherwise, we will ....
