...
predBest <- predict(lmFit1, newdata = selectTest)  %>% as_tibble()
postResample(predBest, obs = selectTest$shares)
```
...
```{r}
rfFit <- train(shares ~. , data = selectTrain %>% dplyr::select('num_videos', 'num_imgs','num_keywords',
...
rfPredict<-predict(rfFit,newdata=selectTest) %>% as_tibble()
RF_Trees<-postResample(rfPredict, obs = selectTest$shares)
